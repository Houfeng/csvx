...
var CSV = new Class({
  ...
  _handleQuote: function (val) {
    return '"' + val.replace(/"/igm, '\\"', 'igm') + '"';
  },
  ...
  stringify: function (objList, headLine, columns) {
    ...
    objList.forEach(function (objItem) {
      ...
      utils.each(columns, function (index, col) {
        ...
        if (utils.isString(val)) val = this._handleQuote(val);
        csvItem.push(val);
      }, this);
    ...
    if (headLine) {
      csvList.unshift(columns.map(function (col) {
        var val = col.displayName || col.name;
        return this._handleQuote(val);
